    ...
    for t in range(5):
        ...
        for l in range(0, Reset_iter):
            ...
            for i in range(0, SearchAgents_no):
                ...
                for j in range(dim):
                    Positions[i, j] = reflect(Positions[i, j], lb[j], ub[j])# - total_shift[j])
                ...
                if numpy.linalg.norm(shift_vector) > 0.05 * numpy.linalg.norm(numpy.array(ub)-numpy.array(lb)):
                    ...
                    for i in range(dim):
                        if SearchAgents_no > 1:  # Only if we have multiple agents
                            ...
                            Positions[0, i] = total_shift[i]  # Alpha stays at origin
            ...
            Convergence_curve[l + (t * Reset_iter)] = Alpha_score

            if l % 1 == 0:
                print(f"Iteration {l + (t * Reset_iter)}: best fitness = {Alpha_score}" + ', ub:' + str(ub[0]) + ", lb:" + str(lb[0]))
