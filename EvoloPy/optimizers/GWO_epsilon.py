...
def GWO_epsilon(objf, lb, ub, dim, SearchAgents_no, Max_iter):
    ...
    # Loop counter
    print('GWO_epsilon is optimizing  "' + objf.__name__ + '"')
    ...
    for l in range(0, Max_iter):
        ...
        for i in range(0, SearchAgents_no):
            for j in range(0, dim):
                ...
                epsilon = 0.1 * (1 - l/Max_iter)
                if l < 0.5 * Max_iter:  # Exploration phase
                    Positions[i] = (X1 + X2 + X3)/3 + epsilon * numpy.random.randn(dim)
