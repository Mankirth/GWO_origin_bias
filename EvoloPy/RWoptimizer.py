...
def selector(algo, func_details, popSize, Iter, OriginShift, Seed, SearchShift):
    function_name = func_details
    lb = RWBench.LowBounds(function_name) + SearchShift
    ub = RWBench.UpBounds(function_name) + SearchShift
    ...
    if algo == "SSA":
        ...
        x = pso.PSO(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    elif algo == "GA":
        ...
        x = de.DE(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    elif algo == "GWOM":
        x = gwo_modified.GWO_modified(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    elif algo == "PSOM":
        x = pso_modified.PSOM(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    elif algo == "ES":
        x = es.ES(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    elif algo == "ESM":
        x = esm.ESM(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    elif algo == "DEM":
        x = de_modified.DE_modified(int(function_name), lb, ub, dim, popSize, Iter, OriginShift, Seed)
    else:
        return None
    return x
    ...
    for i in range(0, len(optimizer)):
        for j in range(0, len(objectivefunc)):
            ...
            for k in range(0, NumOfRuns):
                ...
                x = selector(optimizer[i], objectivefunc[j], PopulationSize, Iterations, OriginShift, k, SearchShift)
